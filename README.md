...
| **Git** | Required to clone [MuJoCo Menagerie](https://github.com/google-deepmind/mujoco_menagerie) |
...
```bash
pip install:
-> mujoco mujoco-python-viewer opencv-python ikpy glfw scipy numpy pillow PyOpenGL sympy
pip install ultralytics opencv-python (used for YOLO)
...
For Main.py, must update: UR5E_XMl, xml_path (scene)
For scene.xml, must update: <include file="..."> for ur5e.xml / 2f85.xml; mesh paths if any
For ur5e xml, must update: meshdir / mesh file= if not relative
For 2f85.xml, must update: compiler meshdir, each <mesh file="...">

Example: UR5E_XML = r"C:\Users\<YOU>\...\mujoco_menagerie\universal_robots_ur5e\ur5e.xml"
...
OpenCV (primary): main path for reliable, colour-agnostic belt objects in this project.
...
Tool Center Point (TCP) / pinch. (TCP: reference point on the tool used for positioning (in this project, the gripper pinch site). Arm motion is planned so that point reaches the aim pose.)
...
**Usage**
Single pick (default-style main): python Main.py
Flow: home → wait for object in camera zone → intercept → pick → place → home.
Phase 4 evaluation: run with the Phase 4 main that loops PHASE4_SUITE, spawns each object, calls the same wait + safe_pick_box_moving, and prints a summary table.
Viewer: MuJoCo passive viewer for the 3D scene, OpenCV window for the overhead camera (if enabled), Close both when the script exits (cv2.destroyAllWindows(); leave the viewer context).

**License / acknowledgements**

MuJoCo — DeepMind / Google
MuJoCo Menagerie — google-deepmind/mujoco_menagerie (UR5e, Robotiq 2F-85 models; check each model’s license)
ikpy, OpenCV, Ultralytics YOLO — respective open-source licenses

This simulation code is intended for research and data collection (e.g. waste-sorting style pick studies). Adapt paths, XML, and gains to your own machine and cell layout.
